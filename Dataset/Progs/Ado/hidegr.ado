...
bys idrh_iecdes idrh_ieesp: egen Highest_Degree1 =max(Highest_Degree)
keep if Highest_Degree == Highest_Degree1
drop Highest_Degree Highest_Degree1

//But we can still have the repetition of the same degree. We decided to keep only the oldest one. We can only do this in the last four years of the data (which have that type of information).
sort idrh_iecdes idrh_ieesp Degree Degree_Year
quietly by idrh_iecdes idrh_ieesp Degree: gen dup = cond(_N==1,0,_n)
drop if dup>1
drop dup
